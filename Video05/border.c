...
void border_draw(const struct Border *b) {
    ...
    dest_rect.y = 0;
    SDL_RenderCopy(b->renderer, b->image, &b->src_rects[0], &dest_rect);

    dest_rect.x = b->piece_width * ((int)b->columns + 1) - b->left_offset;
    dest_rect.y = 0;
    SDL_RenderCopy(b->renderer, b->image, &b->src_rects[2], &dest_rect);

    dest_rect.x = -b->left_offset;
    dest_rect.y = b->piece_width * (int)b->rows + b->piece_height;
    SDL_RenderCopy(b->renderer, b->image, &b->src_rects[5], &dest_rect);

    dest_rect.x = b->piece_width * ((int)b->columns + 1) - b->left_offset;
    dest_rect.y = b->piece_width * (int)b->rows + b->piece_height;
    SDL_RenderCopy(b->renderer, b->image, &b->src_rects[7], &dest_rect);

    for (int r = 0; r < (int)b->rows; r++) {
        dest_rect.x = -b->left_offset;
        dest_rect.y = r * b->piece_width + b->piece_height;
        SDL_RenderCopy(b->renderer, b->image, &b->src_rects[3], &dest_rect);

        dest_rect.x = b->piece_width * ((int)b->columns + 1) - b->left_offset;
        dest_rect.y = r * b->piece_width + b->piece_height;
        SDL_RenderCopy(b->renderer, b->image, &b->src_rects[4], &dest_rect);
    }

    for (int c = 0; c < (int)b->columns; c++) {
        dest_rect.x = (c + 1) * b->piece_width - b->left_offset;
        dest_rect.y = 0;
        SDL_RenderCopy(b->renderer, b->image, &b->src_rects[1], &dest_rect);

        dest_rect.x = (c + 1) * b->piece_width - b->left_offset;
        dest_rect.y = b->piece_width * (int)b->rows + b->piece_height;
        SDL_RenderCopy(b->renderer, b->image, &b->src_rects[6], &dest_rect);
    }
}
